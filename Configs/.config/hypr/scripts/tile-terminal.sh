...
is_dropdown_terminal() {
  local addr="$1"
  hyprctl clients -j | jq -e --arg ADDR "$addr" '
    .[] |
    select(.address == $ADDR) |
    (
      (.class | test("foot|kitty|alacritty|wezterm"; "i"))
    )
  ' >/dev/null 2>&1
}
...
terminal_exists() {
  ...
  if [ -n "$addr" ] && is_dropdown_terminal "$addr"; then
    return 0
  else
    rm -f "$ADDR_FILE"
    return 1
...
terminal_in_special() {
  ...
  if [ -n "$addr" ]; then
    hyprctl clients -j | jq -e --arg ADDR "$addr" '
      any(.[]; .address == $ADDR and .workspace.name == "'"$SPECIAL_WS"'")
    ' >/dev/null 2>&1
  else
    return 1
  fi
...
spawn_terminal() {
  ...
  hyprctl dispatch exec "[float; size $width $height; workspace special:scratchpad silent] $TERMINAL_CMD --title dropdown-terminal"
...
if terminal_exists; then
  ...
  if terminal_in_special; then
    # Show it
    debug_echo "Showing dropdown terminal"
    ...
    # Hide it
    debug_echo "Hiding dropdown terminal"
    ...
    hyprctl dispatch pin "address:$TERMINAL_ADDR" # toggle unpin
